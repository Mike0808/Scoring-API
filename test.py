...
class TestSuite(unittest.TestCase):
    ...
    def set_valid_auth(self, request):
        if request.get("login") == api.ADMIN_LOGIN:
            request["token"] = hashlib.sha512(str(datetime.datetime.now().strftime("%Y%m%d%H") +
    ...
    def test_bad_auth(self, request):
        _, code, _ = self.get_response(request)
    ...
    def test_ok_score_request(self, arguments):
        request = {"account": "horns&hoofs", "login": "h&f", "method": "online_score", "arguments": arguments}
        self.set_valid_auth(request)
        response, code, self.context["has"] = self.get_response(request)
    ...
    def test_ok_interests_request(self, arguments):
        request = {"account": "horns&hoofs", "login": "h&f", "method": "clients_interests", "arguments": arguments}
        self.set_valid_auth(request)
        response, code, self.context['nclients'] = self.get_response(request)
        self.assertEqual(api.OK, code, arguments)
